...
public class Player1Score : MonoBehaviour {
	...
	{
		GameScore.player1Score += 10; //Se le agrega 10 puntos al jugador #1, mandando a llamar la variable player2Score en GameScore.cs
		ball.transform.position = new Vector2(0, 0); //Se reinicia la pelota en el lugar inicial
		player1.transform.position = new Vector2 (-0.05f, -2.78f);  //Se reinicia la posicion de este jugador
		player2.transform.position = new Vector2 (-0.05f, 2.82f);  //Se reinicia la posicion de este jugador
